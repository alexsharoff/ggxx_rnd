 ...
 */
typedef std::function<bool(PEFile*, const wchar_t*)> STUBFUNC;
...
} g_Signatures[]
{
    ...
    { "SteamStub Variant #2", (unsigned char*)"\x53\x51\x52\x56\x57\x55\x8B\xEC\x81\xEC\x00\x10\x00\x00\xC7", "xxxxxxxxxxxxxxx", nullptr },
...
{
    Console::RevertColorsOnExit revert;
    ...
    wchar_t szWorkingDirectory[MAX_PATH] = { 0 };
    wcscpy_s(szWorkingDirectory, argv[1]);
    ::PathRemoveFileSpecW(szWorkingDirectory);

    if (wcslen(szWorkingDirectory) > 0)
        ::SetCurrentDirectoryW(szWorkingDirectory);
    ...
    auto processed = [&]() -> bool
    {
        ...
        for (auto x = 0; x < _countof(g_Signatures); x++)
        {
            ...
            if (found != 0)
            {
                ...
                const auto unpackedPath = std::wstring(argv[1]) + L".unpacked.exe";
                return g_Signatures[x].Func(file, unpackedPath.c_str());
            }
        }
